...
def _load_dataset(data_dir, dataset_name, batch_size, num_classes):
...
def _save_model(model, model_dir, model_id):
    ...
    with open(
        os.path.join(model_dir, f"{model_id}.tflite"), "wb"
...
def train(train_dir, model_dir, batch_size, epochs, monitor):
    ...
    # Load data
    train_dataset = _load_dataset(train_dir, "train", batch_size, num_classes)
    eval_dataset = _load_dataset(train_dir, "eval", batch_size, num_classes)
...
if __name__ == "__main__":
    ...
    # save model
    _save_model(classifier, args.model_dir, model_id)
