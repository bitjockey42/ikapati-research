...
Training was done iteratively, with modifications to the hyperparameters made after each iteration, and models were saved at every checkpoint (in this case, the epoch) for which the validation loss improved. For each of these, epochs was set to 20.

In the benchmark run, the hyperparameters were set to match that of the benchmark model from Toda et al, with the learning rate set to 0.05, the batch size to 128. The architecture was inceptionv3 and the activation function was "relu" (rectified linear units).

In a subsequent iteration, the learning rate was changed to 0.001, the batch size set to 100.
