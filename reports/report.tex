...
This web app has a POST endpoint located at \url{http://jetbot:5000/predict}, which loads the tensorflow lite model and then attempts to predict the class name for an input image.

To simulate a POST request to the Flask endpoint, the curl commandline tool was used with the image file passed in:
...
The 2 best performing models, based on these metrics, were both InceptionV3 networks. In both of these runs, the learning rate was reduced to 0.001 from the benchmark 0.05. 
...
Figure ~\ref{fig:best1} shows how that training run with the batch size reduced to 64 from 128 performed. On the test dataset, this model performed with an accuracy of 97\% and a loss score of 0.089.
...
Figure ~\ref{fig:best2} shows how the training run with the same batch size as the benchmark but a learning rate reduced to 0.001 performed. On the test dataset, this this model performed with an accuracy of 96\% and a loss score of 0.126.
