...
%% TODO: insert distribution plot of classes
...
\textbf{Artificial Neural Networks (ANN)}, which attempt to mimic human brain function in a digital fashion, are well-suited to classification problems. They are comprised of connected nodes that process some input as a distributed node, discern patterns based on the characteristics of that input, and then produce an output corresponding to the class to which the input belongs \cite{oshea_introduction_2015} based on the patterns observed. A particular type of ANN, a \textbf{Convolutional Neural Network (CNN)}, is particularly useful for image classification tasks.

%% What are convolutions in the context of images?

%% How do neural networks incorporate convolutions?

%% Why is this approach used for image problems?

%% What do the hyperparameters mean?

As such, CNN was selected as the classification algorithm due to its demonstrated efficacy in plant disease detection in previous studies \cite{toda_how_2019} \cite{fujita_basic_2016}.
