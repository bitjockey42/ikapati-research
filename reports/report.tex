...
This InceptionV3-based network is based on what was done in a previous study \cite{toda_how_2019}, but as 9 Mixed modules instead of the 11 used in that study.
...
Training was done iteratively, with modifications to the hyperparameters made after each iteration, and models were saved at every checkpoint (in this case, the epoch) for which the validation loss improved.

In a benchmark run, the learning rate was set to 0.001, the batch size to 128, and the activation function to "linear".

In another benchmark run, the hyperparameters were set to match that of the benchmark model from Toda et al, with the learning rate set to 0.05, the batch size to 128, and the epochs to 20. The architecture was inceptionv3 and the activation function was "relu" (rectified linear units).

In subsequent iterations, the learning rate was changed to 0.001, the batch size set to 100.

TODO: insert figures here of the model performance
